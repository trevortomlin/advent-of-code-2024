...
pub fn part_one(input: &str) -> Option<u32> {
    ...
    for i in 0..reports.len() {
        let diff = reports[i].iter().tuple_windows().map(|(&e1, &e2)| (e1 as i32 - e2 as i32).abs()).filter(|&x| x < 1 || x > 3).count() == 0;
...
for i in 0..reports.len() {
    ...
    if is_safe(&reports[i]) && diff {
        ...
        for j in 0..reports[i].len() {
            let new: Vec<u32> = reports[i].iter().enumerate().filter(|(i, _)| *i != j).map(|(_, &e)| e).collect();
            let is_safe_now = new.iter().tuple_windows().map(|(&e1, &e2)| (e1 as i32 - e2 as i32).abs()).filter(|&x| x < 1 || x > 3).count() == 0;
